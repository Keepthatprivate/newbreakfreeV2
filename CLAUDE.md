...
- `src/components/` - UI components (Shadcn/UI in `ui/`, custom in `custom/`)
...
- Use the shared typography components in `@/components/custom/typography.tsx` for paragraphs and headings (instead of creating custom `p`, `h1`, `h2`, etc.).
