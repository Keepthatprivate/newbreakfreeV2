...
FEATURE: Add complete analytics suite - Google Analytics 4, Google Tag Manager, Microsoft Clarity, Meta Pixel, TikTok Pixel, Twitter/X Pixel, LinkedIn Insight Tag, Vercel Analytics, and Speed Insights
CHORE: Update .env-template with all environment variables - Google OAuth, Meta CAPI, Discord webhooks, custom Stripe plans
REFACTOR: Rebrand NowTS to BoilerSaaS - rename src/components/nowts/ to custom/, update all imports, remove @nowts registry

## 2026-01-21

FEATURE: Add Meta Pixel + Conversion API (CAPI) tracking system - client-side Pixel for browser tracking, server-side CAPI for improved attribution, automatic deduplication via event_id, TrackingSession/EventLog/CheckoutLinkage database models for audit, rate limiting, and Stripe webhook integration for Purchase events
FEATURE: Add MetaViewContent component for tracking page views - integrated on landing page and all /start funnel pages
FEATURE: Add /api/meta/capi endpoint with Zod validation, rate limiting, and PII hashing (SHA-256)

## 2026-01-20

REFACTOR: Redesign paywall step 1 & 2 with compelling copywriting, floating orbs, animated counters, scrolling testimonials, chapter breakdowns with page counts, and smooth reveal animations
FEATURE: Add 3-page paywall funnel with Stripe checkout integration - emotional copywriting page, how it works page, and pricing page with monthly ($29.99) and yearly ($59.99) plans
FEATURE: Add opt-in page with frictionless signup flow - direct account creation for new users, OTP only for existing users
FEATURE: Add scrolling testimonials carousel to opt-in page
FEATURE: Add payment success page with animated loading state
FEATURE: Create /start/quiz page with 5-step onboarding form - gender selection, birth date, birth place (auto-detected via IP), birth time with "I don't know" option, palm photo capture with lines overlay - all data persisted in localStorage
REFACTOR: Extract cosmic background and star field into reusable components
REFACTOR: Simplify hero section - remove orbiting zodiac, keep subtle star field and gradient background
REFACTOR: Redesign FAQ section - centered header with subtitle, card-based accordion items with rounded corners, help icons, staggered scroll animations
REFACTOR: Remove testimonials/reviews section from landing page
FIX: Fix buggy animations in features grid - replace whileHover with whileInView for smoother animations, fix hydration errors by using pre-calculated positions, add spring physics and staggered delays for polished reveal effects
FEATURE: Complete landing page redesign inspired by Astroline - new hero section with cosmic background and floating elements, features grid with 6 animated interactive service cards (zodiac wheel, horoscope chart, palm reading lines, compatibility meter, birth chart with rotating planets, tarot card flip), advisors showcase section, how it works timeline, and updated FAQ content for astrology platform
REFACTOR: Remove pricing section and email signup from landing page, redirect all CTA buttons to /start
REFACTOR: Replace fixed navbar with minimal inline header - centered logo with "Already have an account?" link

REFACTOR: Replace Settings with Billing in sidebar - remove settings/members/danger pages from navigation, keep only billing
CHORE: Remove unused orgs-select.tsx and getUsersOrgs query files
REFACTOR: Remove organization selector from sidebar - auto-connect users to their organization
REFACTOR: Complete sidebar redesign - add logo header, polished org selector with avatars, improved navigation menu styling, enhanced user button with initials, better mobile experience with wider sidebar
FIX: Fix dark theme not applying - rename duplicate :root block to .light to prevent overriding dark variables
FIX: Add dark class to html element to prevent light theme flash on initial load
REFACTOR: Modernize sidebar design - remove changelog stack, upgrade card, docs/terms links; rounded buttons with subtle accent colors
REFACTOR: Set default font weight to semibold (600) for bolder text across the app
REFACTOR: Replace SVG logo with image logo (icon.png) in all navigation components
FEATURE: Add Discord notification system with multi-channel webhooks, retry logic, and fire-and-forget pattern for payments, signups, and trial events
REFACTOR: Apply cartoon/rounded design style to all UI components (buttons pill-shaped, cards/dialogs rounded-2xl, inputs/selects rounded-xl, badges/tabs rounded-full)
REFACTOR: Increase base --radius CSS variable from 0.5rem to 1rem for more generous rounded corners
REFACTOR: Add generous spacing to buttons, inputs, textareas and selects (increased height and padding)
REFACTOR: Update fonts to mystical/elegant style - Playfair Display for headings, Lato for body text
REFACTOR: Force dark mode only (remove light mode option)
