...
- 🛑 NEVER lose BoilerSaaS custom CSS (success, warning, info colors)
- ✅ ALWAYS use `git diff` after applying theme to check for lost data
- ✅ ALWAYS restore any deleted BoilerSaaS custom CSS
...
- 📖 Look for deleted lines containing BoilerSaaS, --success, --warning, --info
...
|------------------|------------|
| `/* BoilerSaaS */` | Must restore BoilerSaaS custom colors |
...
```bash
git diff app/globals.css | grep "^-" | grep -E "(BoilerSaaS|--success|--warning|--info|@layer base|nprogress)"
```

2. **Manually add back the missing sections:**

   **BoilerSaaS colors for `:root`** (add before closing `}` of `:root`):
   ```css
   /* BoilerSaaS */
   ...
   **BoilerSaaS colors for `.dark`** (add before closing `}` of `.dark`):
   ```css
   /* BoilerSaaS */
...
✅ No BoilerSaaS custom colors deleted (or restored if deleted)
...
❌ Ignoring deleted BoilerSaaS custom colors in diff
