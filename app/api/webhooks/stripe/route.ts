...
const checkoutSessionCompleted = async (
  ...
  notifySubscriptionCreated({
    subscription: { ...dbSubscription, organization },
    planName: plan.name,
  });

  // Meta Purchase event (fire-and-forget)
  sendMetaPurchaseEvent(session).catch((error) => {
    logger.error("[MetaEvents] Failed to send Purchase event:", error);
  });
  ...
  if (
    ...
    // Discord notification for trial start
    notifyTrialStarted({
      subscription: dbSubscription,
      organizationName: organization.name,
      planName: plan.name,
      trialDays: plan.freeTrial.days,
    });
...
const customerSubscriptionUpdated = async (
  ...
  // Fetch organization for Discord notification
  const organization = await prisma.organization.findUnique({
    where: { id: updatedSubscription.referenceId },
  });

  // Discord notification (fire-and-forget)
  notifySubscriptionUpdated({
    subscription: { ...updatedSubscription, organization },
    previousPlan:
      dbSubscription.plan !== planName ? dbSubscription.plan : undefined,
    newPlan: planName,
  });
  ...
  if (plan?.freeTrial) {
    ...
    if (
      ...
      // Discord notification for trial conversion
      notifyTrialEnded({
...
const customerSubscriptionDeleted = async (
  ...
  // Fetch organization for Discord notification
  const organization = await prisma.organization.findUnique({
    where: { id: dbSubscription.referenceId },
  });

  // Discord notification (fire-and-forget)
  notifySubscriptionCanceled({
    subscription: { ...updatedSubscription, organization },
    planName: plan?.name ?? dbSubscription.plan,
  });
